...
interface FaceSwapSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onPerformSwap: (targetFile: File, sourceFile: File) => void;
  isDarkMode: boolean;
}

type SlotType = 'target' | 'source';
type CropRect = { x: number; y: number; width: number; height: number };
type DragInfo = { type: string; startX: number; startY: number; startRect: CropRect };


const FaceSwapSelectionModal: React.FC<FaceSwapSelectionModalProps> = ({ isOpen, onClose, onPerformSwap, isDarkMode }) => {
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);
  const [targetImage, setTargetImage] = useState<{ file: File, src: string } | null>(null);
  const [sourceImage, setSourceImage] = useState<{ file: File, src: string } | null>(null);
  ...
  const [cropRect, setCropRect] = useState<CropRect>({ x: 0.25, y: 0.25, width: 0.5, height: 0.5 });
  ...
  useEffect(() => {
    return () => {
        if (targetImage && targetImage.src.startsWith('blob:')) {
  ...
  useEffect(() => {
    return () => {
        if (sourceImage && sourceImage.src.startsWith('blob:')) {
        ...
        fetch(capturedImage)
            ...
            .then(blob => {
                if (blob) {
                    ...
                    handleFileChange(cameraView.targetSlot, file);
  ...
  const startCropping = (slot: SlotType) => {
    const imageToCrop = slot === 'target' ? targetImage : sourceImage;
    if (imageToCrop) {
    ...
    img.onload = () => {
        ...
        canvas.toBlob((blob) => {
            if (blob) {
                ...
                if (slot === 'target') {
                  setTargetImage({ file: newFile, src: newSrc });
                } else {
                  setSourceImage({ file: newFile, src: newSrc });
                }
                setView('selection');
            }
        }, 'image/png', 0.95);
  ...
  const handleUseFullImage = () => {
      if (croppingImage) {
          if (croppingImage.slot === 'target') setTargetImage(croppingImage);
          else setSourceImage(croppingImage);
          setView('selection');
      }
  }
  ...
  const ImageSlot = ({ type, image, onSelect, onClear, onCrop }: { type: SlotType, image: {src: string} | null, onSelect: (slot: SlotType) => void, onClear: (slot: SlotType) => void, onCrop: (slot: SlotType) => void }) => (
    <div className="flex flex-col items-center gap-2 w-full sm:w-64">
      <label className="font-title text-primaryText capitalize">{type === 'target' ? 'Corpo (Destino)' : 'Rosto (Origem)'}</label>
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop ${isAnimatingOut ? 'animate-out' : ''}`} onClick={handleClose}>
      <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-6xl max-h-[95vh] flex flex-col modal-content ${isAnimatingOut ? 'animate-out' : ''}`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-borderColor bg-appHeaderBg rounded-t-lg">
          <h2 className="text-xl font-title text-primaryAccent">Criar Troca de Rosto</h2>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
          <p className="text-center text-secondaryText mb-6">A IA irá transplantar o rosto da 'Imagem de Origem' para o corpo na 'Imagem de Destino'.</p>
          <input type="file" ref={fileInputRef} accept="image/*" className="hidden" />
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <ImageSlot type="target" image={targetImage} onSelect={handleFileSelect} onClear={() => setTargetImage(null)} onCrop={startCropping} />
            <ImageSlot type="source" image={sourceImage} onSelect={handleFileSelect} onClear={() => setSourceImage(null)} onCrop={startCropping} />
          </div>
        </div>
        <div className="flex justify-end items-center p-4 border-t border-borderColor">
          <button onClick={handlePerformSwap} disabled={!targetImage || !sourceImage} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition disabled:opacity-50 disabled:cursor-not-allowed">
            Realizar Troca
          </button>
        </div>
        <canvas ref={canvasRef} className="hidden"></canvas>
...
export default FaceSwapSelectionModal;