...
interface AgeChangeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (imageFile: File, currentAge: number, desiredAge: number, mode: 'rejuvenate' | 'age') => void;
  isDarkMode: boolean;
}
...
const AgeChangeModal: React.FC<AgeChangeModalProps> = ({ isOpen, onClose, onConfirm, isDarkMode }) => {
  const [isAnimatingOut, setIsAnimatingOut] = useState(false);
  const [image, setImage] = useState<{ file: File, src: string } | null>(null);
  const [currentAge, setCurrentAge] = useState('');
  const [desiredAge, setDesiredAge] = useState('');
  const [mode, setMode] = useState<'rejuvenate' | 'age'>('age');
  ...
  useEffect(() => {
    ...
    if (!isNaN(ageNum) && ageNum > 0) {
      let newDesiredAge;
      if (mode === 'age') {
        newDesiredAge = ageNum + 10;
      } else { // 'rejuvenate'
        newDesiredAge = Math.max(0, ageNum - 10);
      }
      setDesiredAge(String(newDesiredAge));
    } else if (currentAge === '') {
      setDesiredAge('');
    }
  }, [currentAge, mode]);
  ...
  return (
    <>
      <style>{`
        .switch-container { display: flex; align-items: center; justify-content: center; gap: 0.75rem; font-size: 0.875rem; color: #949597; }
        .switch { position: relative; display: inline-block; width: 50px; height: 28px; }
        .switch input { opacity: 0; width: 0; height: 0; }
        .slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #393B3D; transition: .4s; }
        .slider:before { position: absolute; content: ""; height: 20px; width: 20px; left: 4px; bottom: 4px; background-color: white; transition: .4s; }
        input:checked + .slider { background-color: #970167; }
      ...
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop ${isAnimatingOut ? 'animate-out' : ''}`} onClick={handleClose}>
        <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-2xl max-h-[95vh] flex flex-col modal-content ${isAnimatingOut ? 'animate-out' : ''}`} onClick={e => e.stopPropagation()}>
          <div className="flex justify-between items-center p-4 border-b border-borderColor bg-appHeaderBg rounded-t-lg">
            <h2 className="text-xl font-title text-primaryAccent">Envelhecimento / Rejuvenescimento</h2>
          ...
          <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-6">
            ...
            <div className="flex flex-col items-center gap-2 w-full">
                <label className="font-title text-primaryText">1. Carregue a Foto</label>
                <div className={`relative w-full h-60 border-2 border-dashed border-borderColor rounded-lg flex items-center justify-center ${image ? 'p-0' : 'bg-aiBubble'}`}>
                    {image ? (<><img src={image.src} alt="Imagem para alterar idade" className="w-full h-full object-cover rounded-lg" /><div className="absolute top-1 right-1 flex flex-col gap-1"><button onClick={() => setImage(null)} className="p-1 bg-red-600 text-white rounded-full hover:bg-red-700 transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button><button onClick={() => setView('cropping')} className="p-1 bg-accentBlue text-white rounded-full hover:bg-blue-700 transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 8a6 6 0 016-6v0a6 6 0 016 6v8a6 6 0 01-6 6v0a6 6 0 01-6-6V8z"/></svg></button></div></>) : (<div className="flex flex-col items-center gap-4"><button onClick={handleFileSelect} className="px-4 py-2 bg-inactiveButtonBg text-primaryText rounded-full font-title text-sm hover:bg-gray-600 transition">Galeria</button><button onClick={startCamera} className="px-4 py-2 bg-inactiveButtonBg text-primaryText rounded-full font-title text-sm hover:bg-gray-600 transition">Câmera</button></div>)}
                </div>
            </div>
            
            <div className="space-y-4">
              <label className="font-title text-primaryText text-center block">2. Defina as Idades</label>
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <label htmlFor="current-age" className="block text-secondaryText text-sm mb-1">Idade Atual</label>
                      <input id="current-age" type="number" value={currentAge} onChange={(e) => setCurrentAge(e.target.value)} placeholder="Ex: 25" className="number-input w-full p-2 rounded-lg bg-inputBg text-primaryText border border-borderColor focus:outline-none focus:ring-1 focus:ring-primaryAccent" />
                  </div>
                  <div>
                      <label htmlFor="desired-age" className="block text-secondaryText text-sm mb-1">Idade Desejada</label>
                      <input id="desired-age" type="number" value={desiredAge} onChange={(e) => setDesiredAge(e.target.value)} placeholder="Ex: 50" className="number-input w-full p-2 rounded-lg bg-inputBg text-primaryText border border-borderColor focus:outline-none focus:ring-1 focus:ring-primaryAccent" />
                  </div>
              </div>
              <div className="switch-container">
                <span className={mode === 'rejuvenate' ? 'text-primaryText' : ''}>Rejuvenescer</span>
                <label className="switch">
                    <input type="checkbox" checked={mode === 'age'} onChange={() => setMode(m => m === 'rejuvenate' ? 'age' : 'rejuvenate')} />
                    <span className="slider round"></span>
                </label>
                <span className={mode === 'age' ? 'text-primaryText' : ''}>Envelhecer</span>
              </div>
            </div>

          </div>
          <div className="flex justify-end items-center p-4 border-t border-borderColor">
            <button onClick={handleConfirmAction} disabled={!image || !currentAge || !desiredAge} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition disabled:opacity-50">Gerar Imagem</button>
          </div>
