...
interface FaceTreatmentSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onPerformTreatment: (imageFile: File) => void;
  isDarkMode: boolean;
}
...
const FaceTreatmentSelectionModal: React.FC<FaceTreatmentSelectionModalProps> = ({ isOpen, onClose, onPerformTreatment, isDarkMode }) => {
  ...
  if (view === 'camera') { /* Camera view logic will be similar to other modals */ }
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop ${isAnimatingOut ? 'animate-out' : ''}`} onClick={handleClose}>
      <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-6xl max-h-[95vh] flex flex-col modal-content ${isAnimatingOut ? 'animate-out' : ''}`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-borderColor bg-appHeaderBg rounded-t-lg">
          <h2 className="text-xl font-title text-primaryAccent">Tratamento de Rosto</h2>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex flex-col items-center justify-center gap-6">
            <div className="flex flex-col items-center gap-2 w-full sm:w-80">
                <label className="font-title text-primaryText">Imagem para Tratamento</label>
                <div className={`relative w-full h-80 border-2 border-dashed border-borderColor rounded-lg flex items-center justify-center ${image ? 'p-0' : 'bg-aiBubble'}`}>
                    {image ? (
                    <>
                        <img src={image.src} alt="Imagem para tratar" className="w-full h-full object-cover rounded-lg" />
                        <div className="absolute top-1 right-1 flex flex-col gap-1">
                            <button onClick={() => setImage(null)} className="p-1 bg-red-600 text-white rounded-full hover:bg-red-700 transition">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                            <button onClick={() => setView('cropping')} className="p-1 bg-accentBlue text-white rounded-full hover:bg-blue-700 transition">
                             <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 8a6 6 0 016-6v0a6 6 0 016 6v8a6 6 0 01-6 6v0a6 6 0 01-6-6V8z" /></svg>
                            </button>
                        </div>
                    </>
                    ) : (
        ...
        <div className="flex justify-end items-center p-4 border-t border-borderColor">
          <button onClick={handlePerformTreatment} disabled={!image} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition disabled:opacity-50 disabled:cursor-not-allowed">
            Realizar Tratamento
          </button>
        </div>
...
export default FaceTreatmentSelectionModal;