...
interface AnimateImageModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAnimate: (imageFile: File, prompt: string | null, aspectRatio: '16:9' | '9:16') => void;
  isDarkMode: boolean;
}
...
const AnimateImageModal: React.FC<AnimateImageModalProps> = ({ isOpen, onClose, onAnimate, isDarkMode }) => {
  ...
  if (view === 'cropping' && image) {
      // ... Cropping view JSX (same as other modals)
       return (
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop ${isAnimatingOut ? 'animate-out' : ''}`} onClick={handleClose}>
      <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-6xl max-h-[95vh] flex flex-col modal-content ${isAnimatingOut ? 'animate-out' : ''}`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-borderColor bg-appHeaderBg rounded-t-lg">
          <h2 className="text-xl font-title text-primaryAccent">Animar Imagem com Veo</h2>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-6">
          ...
          <div className="flex flex-col items-center gap-2 w-full">
              <label className="font-title text-primaryText">Imagem Inicial</label>
              <div className={`relative w-full h-60 border-2 border-dashed border-borderColor rounded-lg flex items-center justify-center ${image ? 'p-0' : 'bg-aiBubble'}`}>
                  {image ? (
                  <>
                      <img src={image.src} alt="Imagem para animar" className="w-full h-full object-cover rounded-lg" />
          ...
          {/* Prompt Input */}
          <div>
            <label htmlFor="veo-prompt" className="block text-primaryText text-sm font-medium mb-1">Prompt (Opcional)</label>
            <input 
                id="veo-prompt"
                type="text"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Ex: um drone a sobrevoar a paisagem"
                className="w-full p-2 rounded-lg bg-inputBg text-primaryText border border-borderColor focus:outline-none focus:ring-1 focus:ring-primaryAccent"
            />
          </div>

          {/* Aspect Ratio */}
          <div>
            <label className="block text-primaryText text-sm font-medium mb-2">Proporção do Vídeo</label>
            <div className="flex gap-4">
                <button onClick={() => setAspectRatio('16:9')} className={`flex-1 p-3 rounded-lg font-title transition ${aspectRatio === '16:9' ? 'bg-accentBlue text-white' : 'bg-inputBg text-primaryText hover:bg-gray-700'}`}>16:9 (Paisagem)</button>
                <button onClick={() => setAspectRatio('9:16')} className={`flex-1 p-3 rounded-lg font-title transition ${aspectRatio === '9:16' ? 'bg-accentBlue text-white' : 'bg-inputBg text-primaryText hover:bg-gray-700'}`}>9:16 (Retrato)</button>
            </div>
          </div>

        </div>
        <div className="flex justify-end items-center p-4 border-t border-borderColor">
          <button onClick={handlePerformAnimation} disabled={!image} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition disabled:opacity-50 disabled:cursor-not-allowed">
            Gerar Vídeo
          </button>
        </div>
        <canvas ref={canvasRef} className="hidden"></canvas>
