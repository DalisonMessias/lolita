...
// Re-using crop types from other modals
type CropRect = { x: number; y: number; width: number; height: number };
type DragInfo = { type: string; startX: number; startY: number; startRect: CropRect };

const ChangeBackgroundModal: React.FC<ChangeBackgroundModalProps> = ({ isOpen, onClose, onConfirm, isDarkMode }) => {
  ...
  if (view === 'camera' && !capturedImage) {
    return (
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop`} onClick={handleClose}>
        <div className={`relative bg-black rounded-lg shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col modal-content overflow-hidden`} onClick={e => e.stopPropagation()}>
  ...
  if (view === 'camera' && capturedImage) {
    return (
      <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop`} onClick={handleClose}>
        <div className={`relative bg-appBg rounded-lg shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col modal-content`} onClick={e => e.stopPropagation()}>
          <div className="flex-1 flex items-center justify-center p-4">
            <img src={capturedImage} alt="Capturado" className="max-w-full max-h-full object-contain rounded-lg"/>
          </div>
          <div className="flex justify-center items-center p-4 border-t border-borderColor gap-4">
            ...
            <button onClick={handleUseCapturedPhoto} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition">Usar Foto</button>
          </div>
        </div>
      </div>
    );
             ...
             <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-6xl max-h-[95vh] flex flex-col modal-content`} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 p-6 overflow-hidden relative flex items-center justify-center">
                    <div className="relative inline-block" style={{ userSelect: 'none' }}>
                        <img ref={cropImageRef} src={image.src} alt="Recortar" className="max-w-full max-h-[60vh] object-contain" />
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4 modal-backdrop ${isAnimatingOut ? 'animate-out' : ''}`} onClick={handleClose}>
      <div className={`relative bg-appBg rounded-lg shadow-2xl border border-borderColor w-full md:w-4/5 md:max-w-6xl max-h-[95vh] flex flex-col modal-content ${isAnimatingOut ? 'animate-out' : ''}`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-borderColor bg-appHeaderBg rounded-t-lg">
          <h2 className="text-xl font-title text-primaryAccent">Alterar Fundo da Imagem</h2>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-6">
          ...
          <div className="flex flex-col items-center gap-2 w-full">
              ...
              <div className={`relative w-full h-60 border-2 border-dashed border-borderColor rounded-lg flex items-center justify-center ${image ? 'p-0' : 'bg-aiBubble'}`}>
                  {image ? (
                  <>
                      <img src={image.src} alt="Imagem para alterar fundo" className="w-full h-full object-cover rounded-lg" />
          ...
          {/* Prompt Input */}
          <div>
            <label htmlFor="bg-prompt" className="block text-primaryText text-sm font-medium mb-1">2. Descreva o Novo Fundo</label>
            <textarea 
                id="bg-prompt"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Ex: uma praia tropical ao pôr do sol com palmeiras"
                className="w-full p-2 rounded-lg bg-inputBg text-primaryText border border-borderColor focus:outline-none focus:ring-1 focus:ring-primaryAccent resize-y custom-scrollbar"
                rows={3}
                required
            />
          </div>
        ...
        <div className="flex justify-end items-center p-4 border-t border-borderColor">
          <button onClick={handleConfirmAction} disabled={!image || !prompt.trim()} className="px-6 py-2 bg-primaryAccent text-white rounded-full font-title text-base hover:bg-primaryAccentDark transition disabled:opacity-50 disabled:cursor-not-allowed">
            Gerar Imagem
          </button>
        </div>
